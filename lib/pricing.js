...
function _getOfferProducts(name, region) {
  return _getOffer(name, region).products;
}

function _getOfferTerms(name, region) {
  return _getOffer(name, region).terms;
}
    ...
    a.eval = (filter, type, attributes) => {
      ...
      for (const k in terms) {
        ...
        for (const price of _getSortedPrices(prices)) {
          let qty = (attributes[price.unit] || 0) - consummed;
          ...
          details.push({
            description: `${product.attributes.group || product.attributes.servicename}: ${price.description}`,
            pu: value,
            unit: price.unit,
            total,
